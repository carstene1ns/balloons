	...
	while (!m_quit)
	{
			if(!m_game) 
			{
				...
				else
				{
					...
					{
						if(InputText[CursorPos] == ' ') InputText[CursorPos] = '-';
						else InputText[CursorPos] = ' ';
						menu_timer->reset();
					}
					if(CodeInput)
						EnterCode();
...
do
{
	...
	for(y=16;y<416; y+=32)
	{
		...
		if(prefix == "#") break;
			...
			if(prefix == "&") DrawText(line.substr(1,len),0, y,0);
			else
				if(prefix == "$")
				{
				y-=16;
...
// #ifdef SHOW_DEBUG	// Nur fuer Testzwecke
		...
		if((*game->player.Level = CodeLevel) == 1)				// neuen Startlevel festlegen
			{
				...
				{	
					...
					game->PlayWave(S_BEEP);	// Warnton fuer falschen Code
			}
		else
			game->State=GAME_NEXTLEVEL;
