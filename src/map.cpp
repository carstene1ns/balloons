...
Map::Map()
{
	...
	// Speicher fuer Level und Koll allozieren
...
KollMapCell *pLScreen=pKoll;
	...
	for( ly=0; ly<(YSCROFF+RASTER); ly += RASTER,pLScreen+=wKollLine-xDist)
		...
		{
			if(pLScreen->ebeneU>0)  // Untergrnde zeichenn
...
Koll -= wKollLine;
	...
	{
		if(Koll->ebeneM && y)         		// Mauer drueber
		{
			if(!(Koll-1)->ebeneM && x)		// keine Mauer links drueber
			{
...
// Name:	DrawFigur()	
//
// Desc:	zeichnet die Spielfigur, eventuelle DurchgÃÂ¤nge werden 
//				anschlieÃÂend auch neu gezeichnet
// 
...
int i,loops;
	...
	for(loops = rand() & 7;loops;loops--)
  {
      ...
      {
		    (pMScreen+LastBall)->ebeneG = (pMScreen+i)->ebeneG;		// Ballon an Vorgaengerposition
			  (pMScreen+i)->ebeneG=0;
				LastBall = i;
		  }
  ...
  int AnNum;
	...
	if(sec.elapsed())
	{
		...
		for(AnNum=0 ; AnNum<MAXANIM; AnNum++)
		{
			...
			if(!--AnimListe[AnNum].CycleCounter)	// Zaehler schon Null?
			{
				AnimListe[AnNum].CycleCounter = AnimListe[AnNum].CycleDistance;		// Zyklendistanz
	
				if(AnimListe[AnNum].Counter) // naechster Animationsschritt
...
void Map::SetExitAnimation(void)
{
  ...
  for (i=0;i<MAXANIM;i++)
	  if(AnimListe[i].Tile ==0) break;		// kein Tile dann leer/
