...
void Object_Moving::show(int vx, int vy)
{
	...
	// fÃ¼r das Clipping sorgt Clanlib
...
bool Object_Moving::checkplayerkollision(void)
{
	...
	if(x-20 > game->get_x() + game->map.wScrXoff) return(true);
	else
		if(x < game->get_x() + game->map.wScrXoff-24) return(true);
		else
			if(y-16 > game->get_y() + game->map.wScrYoff) return(true);
			else
				if(y < game->get_y() + game->map.wScrYoff-20) return(true);
...
bool Object_Door::turn(float time_elapsed)
{
  ...
  moveCounter+=time_elapsed*speed;
	...
	if(state == ON)
	{
		...
		if(state == BUSY)		// durch Schalter aus ?
		{
			animCounter=0;
			return(true);	
		}
...
bool Object_Ball::turn(float time_elapsed)
{
  ...
  moveCounter+=time_elapsed*speed;
	...
	switch(direction)
	{
		...
		TestRight:	x+=BALLSPEED;
								if(x & BALLBREAK) break;
								LpKoll = game->map.GetLocalKollPosition(x,y,true);
...
bool Object_Ball::StartBall(KollMapCell *Koll,KollMapCell *EpKoll)
{
  ...
  {
     case W_WERFER:				direction = A_RIGHT;	break;
     case W_WERFER+1: 	direction = A_LEFT;		break;
     case W_WERFER+2: 	direction = A_DOWN;	break;
     case W_WERFER+3: 	direction = A_UP;				break;
  }
